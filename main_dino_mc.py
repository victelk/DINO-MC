...
# Code is modified based on DINO (https://github.com/facebookresearch/dino/blob/main/main_dino.py)
import argparse
import os
import sys
import datetime
...
def get_args_parser():
    ...
    #      ==== new params ====
    parser.add_argument('--data_mode', default='dino', type=str,
                        choices=['dino','mc','tp'],
    ...
    # input bands
    parser.add_argument('--bands', default='RGB_BANDS', type=str)
    ...
    parser.add_argument("--min_scale_crops", type=float, default=0.05, nargs="+",
                        help="argument in RandomResizedCrop (example: 0.05)")
    parser.add_argument("--max_scale_crops", type=float, default=0.32, nargs="+",
                        help="argument in RandomResizedCrop (example: 1.)")
    # Misc
...
def train_dino(args):
    ...
    if args.data_mode == 'mc':
        print('dino-mc')
        transform = DataAugColorMulticrop(
            args.global_crops_scale,
            args.local_crops_scale,
            args.local_crops_number, # customized number of local crops
            args.size_crops,
    ...
    elif args.data_mode == 'tp':
        print('dino-tp')
        transform = DataAugmentationTP(
            args.global_crops_scale,
            args.local_crops_scale,
            args.local_crops_number, # customized number of local crops
            args.size_crops,
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        num_workers=args.num_workers, # how many subprocesses to use for data loading. 0 means that the data will be loaded in the main process.
        pin_memory=True,
        drop_last=True, # True: drop the last incomplete batch, if the dataset size is not divisible by the batch size.
    )
    
    print("Data loaded: there are", len(dataset) , len(data_loader),"images.")
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        print("Unknow architecture:", args.arch)
    ...
    print("Student and Teacher are built: they are both ", args.arch," network.")
    ...
    dino_loss = DINOLoss(
        ...
        args.epochs,
        teacher_crops_number = args.global_crops_number
    ).cuda()
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_output = teacher(images[:args.global_crops_number])  # only the global views pass through the teacher
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, ncrops, warmup_teacher_temp, teacher_temp,
                 warmup_teacher_temp_epochs, nepochs, teacher_crops_number=2, student_temp=0.1,
                 center_momentum=0.9):
        super().__init__()
    ...
    def forward(self, student_output, teacher_output, epoch):
        ...
        teacher_out = teacher_out.detach().chunk(self.teacher_crops_number)
